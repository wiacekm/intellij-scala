...
object InferUtil {
  ...
  def updateTypeWithImplicitParameters(res: ScType, element: PsiElement, coreElement: Option[ScNamedElement], canThrowSCE: Boolean,
                                       searchImplicitsRecursively: Int = 0, fullInfo: Boolean): (ScType, Option[Seq[ScalaResolveResult]]) = DebugLogger.func() {
  ...
  ): (Seq[Parameter], Seq[Compatibility.Expression], Seq[ScalaResolveResult]) = DebugLogger.func() {
